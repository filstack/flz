...
- Invokes `/flz.task` to create implementation plan

**Parallel mode** — work on multiple features simultaneously using `git worktree`:
```
/flz.feature --parallel Add Stripe checkout
```
...
```
/flz.feature --list                          # Show all active worktrees
/flz.feature --cleanup feature/stripe-checkout # Remove worktree and branch
```
...
- Saves plan to `.flz/PLAN.md` (or branch-named file)
- For 5+ tasks, includes commit checkpoints

### `/flz.improve [prompt]`
Refine an existing plan with a second iteration:
```
/flz.improve                                    # Auto-review: find gaps, missing tasks, wrong deps
/flz.improve добавь валидацию и обработку ошибок # Improve based on specific feedback
```
- Finds the active plan (`.flz/PLAN.md` or branch-based `features/<branch>.md`)
- Performs deeper codebase analysis than the initial `/flz.task` planning
...
- If no plan found — suggests running `/flz.task` or `/flz.feature` first
...
- If plan has `Docs: yes` — runs `/flz.docs` after completion
- Offers to delete .flz/PLAN.md when done

### `/flz.verify [--strict]`
Verifies completed implementation against the plan:
```
/flz.verify          # Check all tasks were fully implemented
/flz.verify --strict # Strict mode — zero tolerance before merge
```

**Optional step after `/flz.implement`** — when implementation finishes, you'll be asked if you want to verify.
...
- **Follow-up suggestions** — after verification, suggests running `/flz.security-checklist` and `/flz.review`
...
```
/flz.fix Something is broken    # Choose "Plan first" when asked
```
...
```
/flz.fix                        # Detects FIX_PLAN.md, executes the fix, deletes the plan
```

### `/flz.evolve [skill-name|"all"]`
Self-improve skills based on project experience:
```
/flz.evolve          # Evolve all skills
/flz.evolve fix      # Evolve only /flz.fix skill
/flz.evolve all      # Evolve all skills
```
...
- The more `/flz.fix` patches you accumulate, the smarter `/flz.evolve` becomes
...
- Generates custom skills via `/flz.skill-generator`
- Configures MCP servers

When called with a description:
```
/flz e-commerce platform with Stripe and Next.js
```
...
```
/flz.docs          # Generate or improve documentation
/flz.docs --web    # Also generate HTML version in docs-html/
```
...
**Stays in sync with your code** — when `/flz.feature` asks "Update documentation?" and you say yes, the plan gets `Docs: yes`. After `/flz.implement` finishes all tasks, it automatically runs `/flz.docs` to update documentation. Your docs grow with your codebase, not after the fact.
...
```
/flz.dockerize          # Auto-detect mode based on existing files
/flz.dockerize --audit  # Force audit mode on existing Docker files
```
...
After completion, suggests `/flz.build-automation` and `/flz.docs`.

Supports Go, Node.js, Python, and PHP with framework-specific configurations.

### `/flz.build-automation [makefile|taskfile|justfile|mage]`
Generates or enhances build automation files:
```
/flz.build-automation              # Auto-detect or ask which tool
/flz.build-automation makefile     # Generate a Makefile
/flz.build-automation taskfile     # Generate a Taskfile.yml
/flz.build-automation justfile     # Generate a justfile
/flz.build-automation mage         # Generate a magefile.go
```
...
### `/flz.ci [github|gitlab] [--enhance]`
Generates, enhances, or audits CI/CD pipeline configuration:
```
/flz.ci                   # Auto-detect platform and mode
/flz.ci github            # Generate GitHub Actions workflow
/flz.ci gitlab            # Generate GitLab CI pipeline
/flz.ci --enhance         # Force enhance mode on existing CI
```
...
After completion, suggests `/flz.build-automation` and `/flz.dockerize`.
...
```
/flz.skill-generator api-patterns
```
...
```
/flz.skill-generator https://fastapi.tiangolo.com/tutorial/
/flz.skill-generator https://react.dev/learn https://react.dev/reference/react/hooks
/flz.skill-generator my-skill https://docs.example.com/api
```
...
### `/flz.security-checklist [category]`
Security audit based on OWASP Top 10 and best practices:
```
/flz.security-checklist                  # Full audit
...
/flz.security-checklist csrf             # CSRF protection
/flz.security-checklist secrets          # Secrets & credentials
/flz.security-checklist api              # API security
/flz.security-checklist infra            # Infrastructure & headers
...
```
/flz.security-checklist ignore no-csrf
```
