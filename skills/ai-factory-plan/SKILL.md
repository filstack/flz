...
- **Full** — creates git branch, asks preferences, saves to `.ai-factory/changes/<branch>.md`
...
**Examples:**
```
/ai-factory-plan fast Add product search API
→ mode=fast, description="Add product search API"

/ai-factory-plan full Add user authentication with OAuth
...
/ai-factory-plan --list
→ show worktrees, STOP

/ai-factory-plan --cleanup feature/user-auth
→ remove worktree, STOP

/ai-factory-plan Add user authentication
...
Store all preferences — they will be used in the plan file and passed to `/ai-factory-implement`.
...
```bash
mkdir -p "${WORKTREE}/.ai-factory/changes"
cd "${WORKTREE}"
```
...
To manage worktrees later:
  /ai-factory-plan --list
  /ai-factory-plan --cleanup <branch-name>
```
...
- **Full mode** → `.ai-factory/changes/<branch-name>.md` (replace `/` with `-`)

**Before saving, ensure directory exists:**
```bash
mkdir -p .ai-factory/changes  # only when saving to changes/
```
...
**Full mode + parallel (`--parallel`):** Automatically invoke `/ai-factory-implement` — the whole point of parallel is autonomous end-to-end execution in an isolated worktree.

```
/ai-factory-implement

CONTEXT FROM /ai-factory-plan:
- Plan file: .ai-factory/changes/<branch-name>.md
- Testing: yes/no
- Logging: verbose/standard/minimal
- Docs: yes/no
```
...
1. Check if `<worktree>/.ai-factory/changes/` contains any plan files
...
8. **Plan file location** — Fast mode: `.ai-factory/PLAN.md`. Full mode: `.ai-factory/changes/<branch-name>.md`
...
**Branch-named file** (full mode, e.g., `.ai-factory/changes/feature-user-auth.md`):
...
# -> Look for .ai-factory/changes/feature-user-authentication.md
```
...
-> STOP — user runs /ai-factory-implement when ready
```
...
-> Explores codebase deeply
-> Creates 6 tasks
-> Saves plan to .ai-factory/changes/feature-stripe-checkout.md
-> Auto-invokes /ai-factory-implement (parallel = autonomous)
```
